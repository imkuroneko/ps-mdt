...
<PersonnelSection title="Sospechosos" {onAdd}>
	{#each suspects as suspect}
		{#if !suspect.profileImage}
			<div class="image-warning">
				...
				<span class="warning-text">Sin foto de identificación para <strong>{suspect.fullName}</strong>.</span>
				<div class="warning-actions">
					...
					{#if onTakeMugshot}
						<button
							...
							disabled={!suspect.citizenid}
							type="button"
						title="Tomar foto de identificación (el sospechoso debe estar en línea)"
					>
		...
		{#if !suspect.fingerprint}
			<div class="image-warning fingerprint-warning">
				<svg class="warning-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
				<span class="warning-text">Sin huella dactilar para <strong>{suspect.fullName}</strong>.</span>
				<div class="warning-actions">
					{#if onAddFingerprint}
						<button
							class="warning-action-btn"
							...
							disabled={!suspect.citizenid}
							type="button"
						title="Añadir huella dactilar al registro del sospechoso"
					>
			...
			{#snippet actions()}
				<div class="suspect-actions-row">
					<button
						...
						disabled={!suspect.citizenid}
						type="button"
						aria-label="Emitir orden de detención"
					>
						Emitir Orden de Detención
					</button>
					{#if onIssueBenchWarrant}
						<button
							...
							disabled={!suspect.citizenid}
							type="button"
							aria-label="Emitir orden de banco"
						>
							Emitir Orden de Banco
						</button>
					{/if}
					{#if onIssueBolo}
						<button
